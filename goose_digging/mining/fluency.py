# -*- coding: utf-8 -*-
"""通顺预筛: 判 pair 的 left/right 讲得通不通, 砍明显乱码.

通顺判据宽松 (三选一即通顺):
  (a) 在词典里 / 真实词      —— 词典命中走字典短路 (零 LLM)
  (b) 是一个解释得通的词语    —— 谐音/口语/俚语/略不规范但讲得通 (赶班/撬松)
  (c) 可以作为句子的一部分    —— 能塞进一句话说得通
只有两边都讲得通的 pair 才进评分 (存储所有打分 pair).
(b)/(c) 的判定只对词典外的串送模型 (FLUENCY_SYSTEM 三选一判据).
"""
...
def _judge_unknown(client: openai.OpenAI, strs: list[str],
    ...
    raw = llm_chat(client, FLUENCY_MODEL, system,
                   fluency_prompt(strs),
                   label="预筛", thinking=FLUENCY_THINKING,
                   logger=logger, debug_writer=debug_writer)
    out: dict[str, bool] = {}
    for item in parse_json_list(raw, label="预筛"):
...
def fluency_filter(client: openai.OpenAI, pairs: list[dict],
    ...
    """判每个 pair 的 left+right 讲得通不通, 只保留两边都讲得通的.

    通顺判据宽松 (三选一): 词典里/真实词 (字典短路零LLM), 或解释得通的词语 (谐音/口语),
    或可作为句子的一部分. 盲区词 (赶班/撬松 这类不在词典但讲得通的) 由 LLM 判.
    只有两边都讲得通的 pair 才进评分 (存所有打分 pair).

    词典优先: 在词典的直接判通顺, 只把词典外的送模型.
    ...
    if unknown:
        ...
        for attempt in range(FLUENCY_RETRY_MAX + 1):  # 首判 + N 次重试
            ...
            if attempt < FLUENCY_RETRY_MAX and logger:
                logger(f"\n  [预筛] ⟳ 通顺判漏回 {len(still_missing)}/{len(pending)}, "
        ...
        if final_missing and logger:
            if len(judged) - (len(all_strs) - len(unknown)) == 0:
                logger(f"\n  [预筛] ⚠️ 通顺判重试 {FLUENCY_RETRY_MAX} 次仍一条都没回 "
                       f"({len(unknown)}个送判) —— 输出可能持续被截断/格式错, 本批预筛不可信\n")
            else:
                sample = ",".join(final_missing[:5])
                more = f" 等{len(final_missing)}个" if len(final_missing) > 5 else ""
                logger(f"\n  [预筛] ⚠️ 通顺判重试后仍漏 {len(final_missing)} 个 "
    ...
    if logger:
        ...
        logger(f"\n  [预筛] {len(all_strs)}短语: 词典命中{hit}, 送LLM{len(unknown)}, "
